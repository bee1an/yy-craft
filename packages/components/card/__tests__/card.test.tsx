...
describe('card.tsx', () => {
  ...
  it('card title', () => {
    ...
    expect(wrapper.find(`.${bem.b('header').e('title').value}`).text()).toBe(
      'card title',
    )
  })
  ...
  it('card size', async () => {
    ...
    expect(
      wrapper.element.style.getPropertyValue(createCSSVar('paddingTop')),
    ).toBe('12px')
  ...
  it('bordered', async () => {
    ...
    expect(wrapper.classes()).not.toContain(
      wrapper.vm.bem.b().m('bordered').value,
    )
  })
  ...
  it('hoverable', async () => {
    ...
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('hoverable').value,
    )
  ...
  it('segmented', async () => {
    ...
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('content-segmented').value,
    )
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('footer-segmented').value,
    )
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('action-segmented').value,
    )
    ...
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('content-soft-segmented').value,
    )
    ...
    expect(wrapper.classes()).toContain(
      wrapper.vm.bem.b().m('footer-soft-segmented').value,
    )
  })
})
