...
export const dropdownEmits = {
	/** v-model */
	'update:selected-keys': (() => true) as (newValue: DropdownItem['key'][]) => void
}
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const { styleVars, vars } = useTheme(theme, 'dropdown', dropdownStyle, props)
		...
		const dropdownItems = computed(() => {
			...
			return props.options.map((option) => createItems(option, iconHasWidth, expandHasWidth, null))
	...
	render() {
		...
		const createChild = (item: DropdownItem): JSX.Element | JSX.Element[] => {
			...
			if (item.type === 'group') {
				return [renderItem(item), ...(item.children.map(createChild) as JSX.Element[])]
			}
