...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const { styleVars, vars } = useTheme(theme, 'menu', menuStyle, props, menuLight.exclude)
		...
		const iconSize = computed(() => {
			...
			const sizeKey =
				collapsedWidthDepx < 48 ? 'small' : collapsedWidthDepx < 64 ? 'medium' : 'large'
	...
	render() {
			...
			>
				<div class={bem.b('item').e('indent').value}>
					{Array.from({ length: item.level }).map(() => (
						<div class={bem.b('item').e('indent-box').value} style={{ width: iconMargin }}></div>
					))}
				</div>
		...
		return (
			<div
				...
				ref="menuContainer"
			>
				{collapsed ? sharedEl : <YFadeInExpandTransition group>{sharedEl}</YFadeInExpandTransition>}
			</div>
		)
	}
})
