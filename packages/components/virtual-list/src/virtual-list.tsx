	...
	define() {
		return defineComponent({
			...
			setup(props, { slots, expose }) {
				...
				const scrollPosKey = computed(() => (props.vertical ? 'scrollTop' : 'scrollLeft'))

				const scrollSizeKey = computed(() => (props.vertical ? 'scrollHeight' : 'scrollWidth'))
				const clientSizeKey = computed(() => (props.vertical ? 'clientHeight' : 'clientWidth'))

				const wrapperSizeStyle = computed(() => {
					return {
						[props.vertical ? 'max-height' : 'max-width']: `${props.wrapperMaxSize}px`
					}
				})
				...
				const transformStyle = computed(() => {
					return {
						transform: `translate${props.vertical ? 'Y' : 'X'}(${offset.value}px)`
					}
				})
				// 最小滚动区域尺寸
				...
				const updateItemSize = () => {
					...
					Array.from(visibleZone.value!.children).forEach((child, _index) => {
						...
						if (!sizeGather.value[index] || childSize !== sizeGather.value[index]) {
							minSize.value += childSize - (sizeGather.value[index] || props.itemSize) // 更新撑开滚动区域的元素尺寸
				...
				const render = () => {
					...
					while (true) {
						...
						*/
						if (!sizeGather.value[index] || size - firstChildHiddenSize >= props.maxBound) {
				...
				const sliceData = () => {
					...
					const startKey = (props.data[virtualOption.start] as any)[props.keyField]
					const endKey = (props.data[virtualOption.end - 1] as any)[props.keyField]

					if (startKey === virtualOption.lastStartKey && endKey === virtualOption.lastEndKey) {
						// 未更改不执行
						return
					}
					...
					currentData.value = props.data.slice(virtualOption.start, virtualOption.end)
				...
				const scrollHandler = () => {
					emitter.emit('scroll', {
						...
						maxScrollSize: wrapper.value![scrollSizeKey.value] - wrapper.value![clientSizeKey.value]
					})
...
// 支持slot泛型, 导入时传递泛型
export function GenericVirtualList<T>(): ReturnType<VirtualListFactory<T>['define']> {
	return main
}
