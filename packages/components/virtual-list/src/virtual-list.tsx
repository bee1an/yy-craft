...
import type { ScrollTo } from './use-scroll-to'
    ...
    return defineComponent({
      ...
      setup(props, { slots, expose }) {
        ...
        const wrapperSizeStyle = computed(() => {
          return {
            [props.vertical ? 'max-height' : 'max-width']:
              `${props.wrapperMaxSize}px`,
          }
        })
        ...
        // 记录每个item的size
        const sizeGather = ref<any>(Array.from({ length: props.data.length }))
        // 最小滚动区域尺寸
        ...
        watch(
          ...
          () => {
            nextTick(() => {
              ...
              sizeGather.value = Array.from({ length: props.data.length })
