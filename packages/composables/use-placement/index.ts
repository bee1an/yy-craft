...
export function usePlacement(
  trigger: Readonly<
    ShallowRef<
      HTMLElement | null | { x: number, y: number, w?: number, h?: number }
    >
  >,
  ...
  if (trigger.value && 'x' in trigger.value && 'y' in trigger.value) {
    ...
    ({
      ...
      trigger as Readonly<ShallowRef<HTMLElement | null>>,
    ))
  }
  ...
  const secondPlacement = computed(() => {
    return unref(options).placement.split('-')[1] as
      | 'start'
      | 'end'
      | undefined
  })
