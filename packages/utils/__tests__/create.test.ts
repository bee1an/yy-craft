...
describe('create', () => {
  ...
  it('namespace-block__element--modifier', () => {
    expect(bem.e('icon').m('disabled').value).toBe(
  ...
  it('namespace-block-block__element', () => {
    expect(bem.b(block2).e(element).value).toBe(
      `${namespace}-${block}-${block2}__${element}`,
    )
  })

  it('namespace-block-block--modifier', () => {
    expect(bem.b(block2).m(modifier).value).toBe(
      `${namespace}-${block}-${block2}--${modifier}`,
    )
  })
