		...
		<yy-grid cols="2" :gap="10">
			<yy-gi>
				<yy-flex vertical>
					...
					<yy-card title="延迟触发与延迟隐藏">
						<yy-p>
							设置<yy-text code>delay</yy-text>为延迟触发时间, 设置<yy-text code>duration</yy-text
							>为延迟隐藏时间
						</yy-p>

						<yy-popover trigger="hover" :delay="500" :duration="500">
							<template #trigger><yy-button>延迟500ms触发, 延迟500ms隐藏</yy-button></template>
							<div>{{ text[6] }}</div>
						</yy-popover>
					</yy-card>
					...
					<yy-card title="层级">
						<yy-p>popover有独立的层级计算, 你可以通过<yy-text code>z-index</yy-text>修改它</yy-p>
					...
					<yy-card title="使用包裹元素">
						...
						>
						<yy-p
							>你可能需要<yy-text code>wrapper-class</yy-text>和<yy-text code>wrapper-style</yy-text
							>来定义包裹元素的样式</yy-p
						>

						<yy-p><yy-text type="warning">不要在to不为false下使用, 这将没有任何意义</yy-text></yy-p>
			...
			<yy-gi>
				<yy-flex vertical>
					<yy-card title="触发方式">
						...
						<yy-flex>
							...
							<yy-popover trigger="manual" :show-popover="showPopover">
								<template #trigger>
									<yy-button @click="showPopover = !showPopover">手动</yy-button>
								</template>
								<div>{{ text[4] }}</div>
							</yy-popover>
							<yy-checkbox label="点我手动打开" v-model="showPopover"></yy-checkbox>
						</yy-flex>

						<yy-divider></yy-divider>

						<yy-p style="margin-top: 0"
							>设置<yy-text code>keep-alive-on-hover=false</yy-text>在hover内容元素时同样触发hide,
							仅在<yy-text code>trigger=hover</yy-text>时生效</yy-p
						>
					...
					<yy-card title="宽度">
						<yy-p>
							设置<yy-text code>width</yy-text>为宽度, 当<yy-text code>width=trigger</yy-text
							>时宽度等于触发器宽度</yy-p
						>

						<yy-popover trigger="hover" width="trigger">
							<template #trigger><yy-button>多一点字把这个按钮撑开</yy-button></template>
							<div>{{ text[7] }}</div>
						</yy-popover>
					</yy-card>
					...
					<yy-card title="挂载位置">
						<yy-p
							>popover将默认挂载到body下,可以通过<yy-text code>to</yy-text>挂载到任意位置,
							传递<yy-text code>to=false</yy-text>将挂载到原位置</yy-p
						>
