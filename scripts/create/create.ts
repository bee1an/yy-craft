...
;(async () => {
	...
	useSfc ? createSfc(name, { withStyle, withTheme }) : createTsx(name, { withStyle, withTheme })

	appendFileSync(path.resolve(cmpDir, 'index.ts'), `export * from './${name}'\n`)

	if (withTheme || withStyle) {
		createStyle(name, withTheme)
		appendFileSync(path.resolve(themeDir, 'index.ts'), `export * from './src/${name}'\n`)
	}
})()
